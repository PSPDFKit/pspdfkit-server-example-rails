...
_Note: This example demonstrates the usage of Nutrient Document Engine in a Rails application and is not optimized for production deployments. For information on how to set up Document Engine in production, please refer to [our guides](https://www.nutrient.io/guides/document-engine/deployment/) instead._
...
Make sure to replace `YOUR_ACTIVATION_KEY_GOES_HERE` with your Nutrient Document Engine activation key. You only have to provide the activation key once, after that the server will remain activated until you reset it.
...
- [Nutrient Document Engine](https://www.nutrient.io/getting-started/document-engine/)
  running on [http://localhost:4000](http://localhost:4000) with the default configuration
...
Login using any user name and upload a PDF, then click on the cover image to see Nutrient Web SDK in action.
