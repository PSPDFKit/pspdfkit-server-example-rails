...
module PSPDFKit
  ...
  def upload_document(path)
    # call the PSPDFKit Document Engine api endpoint "POST api/documents/" to upload a new document
  ...
  def delete_document(document)
    ...
    #  1. We delete the document from PSPDFKit Document Engine using the DELETE endpoint.
    #  2. When the deletion was successful, we also remove the database entry.
    #
    # This way, we can be sure that we don't remove a document from our database that is not yet removed
    # from the Server.
    response = client["api/documents/#{document.server_id}"].delete
    # When PSPDFKit Document Engine returns status code 200 OK delete the document from the database.
  ...
  def document_layers(document)
    ...
    layers = JSON.parse(response.body)["data"]

    # Inject default layer
    layers.insert(0, "")
  end
  ...
  def create_layer(document, name)
    ...
    client["api/documents/#{document.server_id}/layers"].post(
      { name: name }.to_json,
